...
import { PlayerId, type SavedDeck, type Element, type CardData } from '@/lib/types';
  ...
  const renderContent = () => {
    ...
    if (!selectedDeck) {
        ...
        if (selectionMode === 'starter') {
             return (
                <>
                    <DeckSelectionModal decks={StarterDecks.map(deck => localizeDeck(deck))} onDeckSelect={handleDeckSelect} />
        ...
        if (selectionMode === 'custom') {
            if (customDecksForSelection.length === 0) {
                return (
                     <div className="w-full h-full flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm gap-8 p-8">
             ...
             return (
                <>
                    <DeckSelectionModal decks={customDecksForSelection} onDeckSelect={handleDeckSelect} />
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-50">
                         <Button variant="tcg" onClick={() => setSelectionMode(null)}>
                            <ArrowLeft className="mr-2 h-4 w-4" />
                            {t('backToTypeSelection')}
    ...
    if (startingPlayer) {
      return <GameBoard playerDeck={selectedDeck} startingPlayer={startingPlayer} onReset={handleReset} isTutorial={false} />;
    }
    return null; // Should not happen
  }
  
  const gameState = useMemo(() => {
    if (isLoading || !selectedDeck || showCoinToss || !startingPlayer) {
      return 'setup';
    }
    return 'playing';
  }, [isLoading, selectedDeck, showCoinToss, startingPlayer]);
