...
const setCards = MASTER_DB.filter((c) => c.set === SET_NAME).sort(
  (a, b) => a.numberInSet - b.numberInSet
);
...
const RARE_PLUS_RARITY_FULL: Partial<Record<Rarity, number>> = {
  ...
  Rare:
    1 -
    (RARE_PLUS_RARITY.Epic! +
      RARE_PLUS_RARITY.Legendary! +
      RARE_PLUS_RARITY.GOD!),
};
...
const countBy = (pool: CardData[], rarity: Rarity, element?: Element) =>
  pool.filter((c) => c.rarity === rarity && (!element || c.element === element))
    .length;
...
const calculateChancePerCard = (card: CardData): string => {
  ...
  if (type === "Aether") {
    ...
    if (numAetherOfRarity > 0) {
      chance +=
        PACK.aetherSlot *
        ((AETHER_SLOT_RARITY[rarity] ?? 0) / numAetherOfRarity);
    }
    // Legendary/GOD Aether-type cards can also drop from the Rare+ slot.
    if (rarity === "Legendary" || rarity === "GOD") {
      chance +=
        PACK.rarePlusSlot *
        (RARE_PLUS_RARITY_FULL[rarity] ?? 0) *
        slotChance(card, true);
    }
  } else if (rarity === "Common" || rarity === "Uncommon") {
    chance +=
      PACK.commonSlots *
      (COMMON_SLOT_RARITY[rarity] ?? 0) *
      slotChance(card, false);
...
export default function AwakeningSetPage() {
  const [inspectedCard, setInspectedCard] = useState<InspectedCard | null>(
    null
  );
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold title-gradient uppercase">
            {t("cardSetTitle", { name: SET_NAME })}
          </h1>
          <p className="text-muted-foreground mt-2">
            {t("cardsInSet", { count: setCards.length })}
          </p>
        </div>
          ...
          {setCards.map((card) => (
            ...
            >
              ...
              <div className="text-center text-xs">
                <p className={`font-bold ${getRarityColor(card.rarity)}`}>
                  {rarity(card.rarity)}
                </p>
