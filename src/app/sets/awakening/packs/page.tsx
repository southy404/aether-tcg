...
const elementPacks: {element: Element, img: string, hint: string}[] = [
    { element: 'Feuer', img: '/set/awakening/booster-awakening-fire.png', hint: 'fire booster pack' },
    { element: 'Wasser', img: '/set/awakening/booster-awakening-water.png', hint: 'water booster pack' },
    { element: 'Aether', img: '/set/awakening/aether-booster-new.png', hint: 'aether booster pack' },
    { element: 'Erde', img: '/set/awakening/booster-awakening-earth.png', hint: 'nature booster pack' },
    { element: 'Luft', img: '/set/awakening/booster-awakening-air.png', hint: 'air booster pack' },
];

const PackDisplay = ({ pack, onClick }: { pack: typeof elementPacks[0], onClick: () => void }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const rX = ((e.clientY - rect.top - rect.height / 2) / (rect.height / 2)) * -6; // Reduced rotation
    const rY = ((e.clientX - rect.left - rect.width / 2) / (rect.width / 2)) * 6;  // Reduced rotation
    ...
    <>
      <AlertDialog open={showBuyDialog} onOpenChange={setShowBuyDialog}>
        <AlertDialogContent className="bg-background/95 border-primary max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl text-center">{t('packBuy')}</AlertDialogTitle>
            <AlertDialogDescription className="text-center">
              {t('choosePayment', { pack: packToBuy ? element(packToBuy) : '' })}
          ...
          <div className="grid grid-cols-2 gap-4 py-6">
            ...
            >
                <div className="flex items-center gap-2 text-yellow-500">
                    <Image src="/coin.png" alt="Gold" width={20} height={20} />
            ...
            >
                <div className="flex items-center gap-2 text-cyan-400">
                    <Image src="/gem.png" alt="Merits" width={20} height={20} />
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="w-full" onClick={() => setPackToBuy(null)}>{t('cancel')}</AlertDialogCancel>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      <div className="container mx-auto flex flex-col items-center justify-center p-4 py-12 text-center">
        <h1 className="mb-4 text-5xl font-bold tracking-wider title-gradient uppercase">{t('cardPacks')}</h1>
        <p className="text-lg text-muted-foreground mb-12">{t('acquireCards')}</p>
        
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-8 gap-y-4 max-w-7xl w-full">
            {elementPacks.map((pack) => (
                <div key={pack.element} className="flex flex-col items-center gap-2">
                    <PackDisplay pack={pack} onClick={() => handleBuyClick(pack.element)} />
                        ...
                        <Dialog>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>{t('awakeningPackStructure')}</DialogDescription>
                            </DialogHeader>
                            <div className="text-sm space-y-4">
                                <div>
                                    <h3 className="font-bold mb-2">{t('packStructure')}</h3>
                                    <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                                        <li><span className="font-semibold text-foreground">{t('aetherCard')}</span> {t('aetherCardDescription')}</li>
                                        <li><span className="font-semibold text-foreground">{t('commonUncommonCards')}</span> {t('commonUncommonCardsDescription')}</li>
                                        <li><span className="font-semibold text-foreground">{t('rarePlusCard')}</span> {t('rarePlusCardDescription')}</li>
                                    </ul>
                                </div>
                                <div>
                                     <h3 className="font-bold mb-2">{t('elementFocus')}</h3>
                                     <p className="text-muted-foreground">{t('elementFocusDescription')}</p>
                                </div>
                                 <div>
                                    <h3 className="font-bold mb-2">{t('rarityOdds')}</h3>
                                    <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                                        <li><span className="font-semibold text-green-400">{t('uncommonOdds')}</span></li>
                                        <li><span className="font-semibold text-purple-400">{t('epicOdds')}</span></li>
                                        <li><span className="font-semibold text-yellow-400">{t('legendaryOdds')}</span></li>
                                        <li><span className="font-semibold text-red-500">{t('godOdds')}</span></li>
                                    </ul>
                                </div>
                            </div>
                          </DialogContent>
